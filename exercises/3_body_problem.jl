...
println("Done.\nCreating images...")
#--- THE ANIMATION CREATION IS VERY SLOW, FOR NOW I'LL SAVE ONLY SOME FRAMES ---
len=length(tis)
step=div(len,100)
for i in 1:step:len
    t_round=round(tis[i],digits=2)
    scatter([uis[1][i]],[uis[2][i]],color=:deepskyblue,title="t=$t_round",titlefontsize=15,legend=false)
